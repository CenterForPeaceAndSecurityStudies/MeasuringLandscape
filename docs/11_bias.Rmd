...
This file demonstrates that the kinds of locations that are imputed are different from the true locations, in terms of things like population, distance from roads, ruggeness, etc.
...
georef_all_dt_covariates_events_sf$rain <- new_over(georef_all_dt_covariates_events_sf , covariate_list[['raster_rain']]  , 'Tribe' )
georef_all_dt_covariates_events_sf$population <- new_over(georef_all_dt_covariates_events_sf , covariate_list[['pop_raster_roi']]  , '' )
georef_all_dt_covariates_events_sf$treecover <- new_over(georef_all_dt_covariates_events_sf , covariate_list[['forest_raster_roi']]  , '' )
georef_all_dt_covariates_events_sf$ruggedness <- new_over(georef_all_dt_covariates_events_sf , covariate_list[['ruggedness_raster_roi']]  , '' )
georef_all_dt_covariates_events_sf$roads_distance <- new_over(georef_all_dt_covariates_events_sf , covariate_list[['roads_distance_to']]  , '' )
...
setkey(georef_all_dt, "rule_ensemble") #sort by ensemble score, pick the best option for each source
...
p9 <- ggplot(temp, aes(x = value,
                 ...
                 geom_vline(xintercept=1) + ylab("Source Dataset") +
                 xlab("Ratio of Mean Value at Imputed Location Relative to True Location") +
                 scale_shape_manual(values=1:15) + theme_bw() + theme(legend.position="none") 
p9
ggsave(
  filename = glue(dir_figures, "p_bias_in_covariates_by_source.pdf"),
