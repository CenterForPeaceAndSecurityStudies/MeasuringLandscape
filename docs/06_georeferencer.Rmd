...
This file performs georeferencing of locations in the events data. It produces an event-location file, with multiple possible matches organized by different rules and levels of confidence. This file can then be subset however you like based on a given set of rules.
...
We load the cobined gazeteer files.
...
When we start, we have `r length(unique(flatfiles_sf_roi$name_cleaner))` unique location strings in the gazeteer data.
...
If we considered every single pair of possible matches it would require about 62 million comparisons (`r length(unique(events_sf$name_cleaner)) * length(unique(flatfiles_sf_roi$name_cleaner))`). And that is just for our relatively small number of events location strings and restricting our gazeteers to relatively small region of interest. Obviously, and approach we impliment is going to have to scale sub-quadratic in the number of locations or it won't be applicable for any problem of interesting size.
...
fromscratch=F # For some reason these parallized functions hang in rmarkdown. Copy and paste them into console to run.
if(fromscratch){
  ...
  buckets_list <- foreach(q = levels(cuts)) %dopar% lsh(corpus_ab_spaced[cuts %in% q], bands = bands, progress = T)
  ...
  saveRDS(candidates, glue(getwd(), "/inst/extdata/candidates.Rdata")) #Leaving off the .. because you should only be running this by hand
}

```

There are `r nrow(candidates)` candidate stem matches return by locality sensitve hashing with these parameters. This is a tiny `r round(nrow(candidates)/length(stemmed_ab)^2,4)` fraction of all the possible stem to stem matches we could have considered.
...
glue(stemmed_ab_suggestions %>% nrow() , " string matches")
...
glue(stemmed_ab_suggestions_events %>% nrow() , " string matches")
...
# Screen pairs that are too disimilar



```{r}
ab_suggestions_events[,temp_q_cos:= stringsim(name_cleaner_a,name_cleaner_b,"cos", nthread=detectCores(),q=2),] #both fast and very good at sorting
ab_suggestions_events <- ab_suggestions_events[temp_q_cos>.3] ; #dim(ab_suggestions_events) #less than .3 is never match
glue(ab_suggestions_events %>% nrow() , " string matches after removing those with too small a ngram2 cosine distance")
...
saveRDS(ab_suggestions_directed,
        file=glue(getwd(), "/../inst/extdata/ab_suggestions_directed.Rds") 
        )
...
ab_suggestions_directed <-  readRDS(system.file("extdata", "ab_suggestions_directed.Rds", package = "MeasuringLandscapeCivilWar")) 
...
#georef2 is a combination of events, and all the gazeteer names they might match to
...
#georef3 is a combination of gazeteers and all gazeteer names they might match to
...
#georef all is a mapping of events, to gazeteer names, to 
georef_all <- georef2 %>% 
...
saveRDS(georef_all_dt,
        file=glue(getwd(), "/../inst/extdata/georef_all_dt.Rds")
)
