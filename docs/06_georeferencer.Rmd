...
buckets_list <- foreach(q = levels(cuts)) %dopar% lsh(corpus_ab_spaced[cuts %in% q], bands = bands, progress = T) , mc.cores = n.cores)

#p_load(multicore)
n.cores <- detectCores()-1
cuts <- cut(1:length(corpus_ab_spaced), n.cores)
buckets_list <- mclapply(levels(cuts),
               function(q) lsh(corpus_ab_spaced[cuts %in% q], bands = bands, progress = T) ,
               mc.cores = n.cores)
...
#Qgram hash the stems, size 3
stemmed_ab_grams <- dfm(
                quanteda::tokens(stemmed_ab, what = "character",
                                 ngrams = 1:3,
                                 skip = 0, 
                                 concatenator = "_",
                                 hash = F)
                )
dim(stemmed_ab_grams)
stemmed_ab_grams <- as(stemmed_ab_grams, "dgCMatrix")
class(stemmed_ab_grams)
rownames(stemmed_ab_grams) <- stemmed_ab    
...
stemmed_ab_grams_scaled <- stemmed_ab_grams
stemmed_ab_grams_scaled@x <- stemmed_ab_grams_scaled@x / rep.int(colSums(stemmed_ab_grams_scaled), diff(stemmed_ab_grams_scaled@p))
dim(stemmed_ab_grams_scaled)
...
pairs <- get_similar_pairs_cosine(
                                  stemmed_ab_grams_scaled,
                                  bands_number = 32, # increase this number, and you'll get more false positives but fewer misses
                                  rows_per_band = 8, # basically shrink this number
...
#18,979,965  for 32 and 8 
#18979965/17880 = 1061.519 about a thousand suggestions per
stemmed_ab_suggestions <- lhs_getpairs(strings=stemmed_ab,
                                       grams=stemmed_ab_grams_scaled,
                                       bands_number=32,
                                       rows_per_band=8) ; dim(stemmed_ab_suggestions)
