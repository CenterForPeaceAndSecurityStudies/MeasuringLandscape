...
events <- readRDS(system.file("extdata", "MeasuringLandscapeCivilWar_events_cleaned.Rdata", package = "MeasuringLandscapeCivilWar")) %>% 
             mutate(event_hash = apply(., 1, digest, algo="xxhash64") ) #create a hash id to referec

dim(events)

table(!is.na(events$location_text),
      !is.na(events$map_coordinate_clean_latitude))
library(dplyr)
library(plyr)
events_text_unique <- ddply(events, .(location_text), summarize,
      map_coordinate_has =sum(!is.na(map_coordinate_clean_latitude))
      )
table(events_text_unique$map_coordinate_has>0)
