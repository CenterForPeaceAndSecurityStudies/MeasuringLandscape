...
glue("Military coordinates only")
y <- !is.na(events_sf$map_coordinate_clean_latitude); table(y)
pred_cords <- predict_missingness_dv(y)
...
table(y)
pred_source_handrule <- predict_missingness_dv(y)
...
table(y)
pred_source_ensemble <- predict_missingness_dv(y)
...
for( q in q_all ){
  ...
  pred_fuzzy_list[[as.character(q)]] <- predict_missingness_dv(y)
}
...
for( q in na.omit(unique(georef_all_dt_bysource$source_dataset)) ){
  ...
  pred_source_dataset_list[[as.character(q)]] <- predict_missingness_dv(y)
}
...
for( q in na.omit(unique(georef_all_dt_bygeometry_type$geometry_type)) ){
  ...
  pred_geometry_type_list[[as.character(q)]] <- predict_missingness_dv(y)
}
...
for( q in na.omit(unique(georef_all_dt_byselfreference$SelfReference)) ){
  ...
  pred_selfreference_list[[as.character(q)]] <- predict_missingness_dv(y)
}
...
bias_dv_df <- rbindlist(list(
    ...
    cbind(auc=auc_selfreference, recall=recall_selfreference) %>% data.frame() %>% 
      rownames_to_column("label") %>% mutate(label=ifelse(label, "Match to Other Events","No Match to Other Events")) %>% 
                        mutate(Type="Allow Match To Other Events"),
    
    cbind(auc=auc_fuzzy, recall=recall_fuzzy) %>% data.frame() %>% rownames_to_column("label") 
    ...
    cbind(auc=auc_source_dataset, recall=recall_source_dataset) %>% data.frame() %>% 
      rownames_to_column("label") %>% mutate(Type="Source Dataset"),
    
    cbind(auc=auc_geometry_type, recall=recall_geometry_type) %>% data.frame() %>% 
      rownames_to_column("label") %>% mutate(Type="Geometry Type")
))

saveRDS(bias_dv_df,
      file=glue(getwd(), "/../inst/extdata/bias_dv_df.Rds")
)
...
#bias_dv_df <- readRDS(system.file("extdata", "bias_dv_df.Rds", package = "MeasuringLandscapeCivilWar"))
...
ggsave(
  ...
  plot = p_bias_dv, width = 12, height = 6,
